...
const columns: KanbanColumn[] = [
  {
    ...
    title: 'Задачи этапа',
    hint: 'Выбор приоритета и подразделения',
  },
  {
    statuses: ['ASSIGNED', 'READY_FOR_WORK'],
    title: 'На распределении',
    hint: 'Назначение исполнителя',
  },
  {
    statuses: ['IN_PROGRESS'],
    title: 'В процессе',
    hint: 'Задача в работе',
  },
  {
    statuses: ['DEPARTMENT_REVIEW', 'CHIEF_DOCTOR_REVIEW'],
    title: 'На согласовании',
    hint: 'Проверка и финальное решение',
  },
  {
    statuses: ['DONE'],
    title: 'Готово',
            ...
            return (
              ...
              >
                ...
                {expanded ? (
                  <div className="kanban-card-details">
                    <p>
                      {severityLabels[card.severity]} проблема · кабинет {card.roomCode}. Работа идёт через чат и список.
                    </p>
                    {renderFailedAttemptPanel(card)}
                    {renderEscalationPanel(card)}
                    <dl className="kanban-card-facts">
...
function getRolePanelText({
  ...
  if (readOnly) {
    return {
      title: 'Обзор команды',
      description: `Все карточки команды: ${visibleCount}. Только просмотр.`,
    };
  }
  ...
  return {
    title: 'Мои задачи',
    description: showingAllCards
      ? `Все карточки команды: ${totalCount}. Действия только по вашим задачам.`
      : visibleCount
      ? 'Ваши задачи: в работу и на согласование.'
